...
int main()
{
    ...
    while (uchoice == 1)
    {
        ...
        printf("If you want to continue, type 1 (yes), or any other number (no): ");
        scanf("%d", &uchoice);  // prompt again to continue or not
    }
...
char menu()
{
    ...
    printf("3. Exit\n");
    return '0'; // '0' used to satisfy char return type, though unused
}
...
char registration() 
{
    ...
    if (stno + temp > 5)
    {
        printf("Error: You can only register %d more student(s).\n", 5 - stno);
        return '0';
    }
